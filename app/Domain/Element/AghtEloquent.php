<?php
namespace App\Domain\Element;


use App\Domain\Departure\DepartureEloquent;
use App\Domain\MasterData\VendorEloquent;
use App\Domain\System\RoleEloquent;
use App\Help\Domain\Timezone;
use App\Infrastructure\Persistence\Eloquents\BaseEloquent;
use App\Infrastructure\Persistence\Eloquents\Contracts\Element\IAghtEloquent;
use App\Infrastructure\Persistence\Eloquents\Contracts\MasterData\IVendorEloquent;
use App\Infrastructure\Persistence\Eloquents\Contracts\System\IRoleEloquent;
use DateTime;
use Illuminate\Database\Eloquent\Relations\Relation;
use Illuminate\Database\Eloquent\SoftDeletes;
use Illuminate\Notifications\Notifiable;
use Illuminate\Support\Facades\Config;

/**
 * @OA\Schema(
 *     schema="AghtEloquent",
 *     title="Aght Eloquent",
 *     description="Aght eloquent schema"
 * )
 */
class AghtEloquent extends BaseEloquent implements IAghtEloquent
{
    use Notifiable, SoftDeletes;

    protected $table = IAghtEloquent::TABLE_NAME;
    protected $primaryKey = 'id';
    protected $fillable = [
        'departure_id', 'aght', 'reason', 'init', 'aghtable_id', 'aghtable_type',
        'created_by', 'created_at', 'updated_by', 'updated_at'
    ];
    protected $searchable = [
        'departure_id', 'aght', 'reason', 'init', 'aghtable_id', 'aghtable_type',
        'created_by', 'created_at', 'updated_by', 'updated_at'
    ];
    protected $orderable = [
        'departure_id', 'aght', 'aghtable_id', 'aghtable_type',
        'created_at', 'updated_at'
    ];
    public $timestamps = false;

    /**
     * @OA\Property(
     *     property="id",
     *     type="integer",
     *     format="int64",
     *     description="Id property",
     *     example=1
     * )
     */

    /**
     * @OA\Property(
     *     property="departure_id",
     *     type="integer",
     *     format="int64",
     *     description="Departure id property"
     * )
     */
    public function getDepartureId(): int
    {
        return $this->departure_id;
    }

    public function setDepartureId(int $departure_id)
    {
        $this->departure_id = $departure_id;
        return $this;
    }

    /**
     * @OA\Property(
     *     property="aght",
     *     type="string",
     *     format="date-time",
     *     description="Aght property",
     *     example="YYYY-MM-DD HH:MM:SS"
     * )
     * @throws \Exception
     */
    public function getAght(): DateTime
    {
        return new DateTime($this->aght);
    }

    public function setAght(DateTime $aght)
    {
        $this->aght = Timezone::convertSetDatetime($aght->format(Config::get('datetime.format.database_datetime')));
        return $this;
    }

    /**
     * @OA\Property(
     *     property="reason",
     *     type="string",
     *     description="Reason property"
     * )
     */
    public function getReason(): string
    {
        return $this->reason;
    }

    public function setReason(string $reason)
    {
        $this->reason = $reason;
        return $this;
    }

    /**
     * @OA\Property(
     *     property="init",
     *     type="bool",
     *     description="Init property"
     * )
     */
    public function getInit(): bool
    {
        return $this->init;
    }

    public function setInit(bool $init)
    {
        $this->init = $init;
        return $this;
    }

    /**
     * @OA\Property(
     *     property="aghtable_id",
     *     type="integer",
     *     format="int64",
     *     description="Aghtable id property"
     * )
     */
    public function getAghtableId(): int
    {
        return $this->aghtable_id;
    }

    public function setAghtableId(int $aghtable_id)
    {
        $this->aghtable_id = $aghtable_id;
        return $this;
    }

    /**
     * @OA\Property(
     *     property="aghtable_type",
     *     type="string",
     *     description="Aghtable type property"
     * )
     */
    public function getAghtableType(): string
    {
        return $this->aghtable_type;
    }

    public function setAghtableType(string $aghtable_type)
    {
        $this->aghtable_type = $aghtable_type;
        return $this;
    }


    public function getAghtAttribute($value)
    {
        return Timezone::convertGetDatetime($value);
    }


    public function departure()
    {
        return $this->belongsTo(DepartureEloquent::class, 'departure_id');
    }

    public function aghtable()
    {
        return $this->morphTo();
    }
}

Relation::morphMap([
    IRoleEloquent::MORPH_NAME => RoleEloquent::class,
    IVendorEloquent::MORPH_NAME => VendorEloquent::class
]);
